...
class Neural_Network:
    ...
    def train(self, epochs, eta, minibatch_size, log_file, dropout_rate=0, dropout=False):
        # constants
        ...
        for e in tqdm(range(epochs)):
            ...
            train_error = self.test(train=True, dropout_rate=dropout_rate, dropout=dropout)
            test_error = self.test(train=False, dropout_rate=dropout_rate, dropout=dropout)
    ...
    def test(self, train, dropout_rate=0, dropout=False):
        # check test/training
        if train:
...
if __name__ == "__main__":
    ...
    f.flush()
    train_errors, test_errors = network.train(epochs, learning_rate, minibatch_size, f, dropout_rate=dropout_rate, dropout=True)
    f.close()
