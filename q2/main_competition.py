...
class Neural_Network:
    ...
    def train(self, epochs, eta, minibatch_size, log_file, a, b):
        # constants
        ...
        for e in tqdm(range(epochs)):
            ...
            log_file.write('%d\tTrain:%f\tTest:%f\teta:%f\n' % (e+1, train_error, test_error, eta))
            if e % 100 == 0:
                log_file.flush()
...
if __name__ == "__main__":
    ...
    f.flush()
    train_errors, test_errors = network.train(epochs, learning_rate, minibatch_size, f, a, b)
    f.close()
    for i in range(len(network.weights)):
        np.savetxt('weight' + str(i), network.weights[i])
